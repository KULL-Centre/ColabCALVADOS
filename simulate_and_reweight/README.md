...
Explore how varying the regularization parameter $\theta$ affects (i) the quality of the SAXS fit (quantified by $\chi^2$) and (ii) how strongly the frame weights deviate from the initial uniform weights (quantified the effective number of frames). Do you agree with the automatically chosen $\theta$, or would you choose a higher or lower value?
17. In "10. Analyze reweighted ensemble", This cell shows comparisons between SAXS curves and conformational properties from <font color='#808080'>experiments (grey) </font> and from the simulation trajectory <font color='#058ED9'>before (blue) </font> and after BME <font color='#d42828'>reweighting (red) </font>. 
18. Estimate the relative change in $R_g$, $\Delta R_g / R_g$, and the absolute change in $\nu$, $\Delta \nu$, when going from the starting simulation ensemble to the BME-reweighted ensemble. Which of these two changes is numerically larger? Is this expected based on the scaling relation $R_g \propto N^{\nu}$, assuming that the prefactor remains unchanged?
19. As an exercise, go back to cell 5 and check the "Break_CALVADOS" box. That will add random noise to the amino acid stickiness parameters of the CALVADOS model ($\lambda$ values). The resulting force field will likely not reproduce the experimental data accurately and the effect of reweighting will be more evident. 
20. In "11. Download results", you can download all the relevant files:
